function is_valid_QCFO = is_QCFO(Wr, dims, parties, tol)
%is_QCFO determines if input is a QC-FO process
%   is_valid_QCFO = is_QCFO(Wr, dims, parties, tol) is true if W is a valid QC-FO (including superinstrument)
...
% Written by Alastair Abbott, last modified 19 August 2022
    ...
    if isa(W,'sdpvar')
        is_valid_QCFO = [trace(W) == d_O, superop_in_QCFO_cone(Wr,dims,parties,tol)];
    else
        is_valid_QCFO = (abs(trace(W) - d_O) <= tol) && superop_in_QCFO_cone(Wr,dims,parties,tol);
    end
end
