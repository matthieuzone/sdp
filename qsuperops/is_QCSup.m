function is_valid_QCSup = is_QCSup(Wr, dims, parties, tol)
%is_QCSup determines if input is a Quantum circuit with Superposition of orders
%   is_valid_QCSup = is_QCSup(W, dims, parties, tol) is true if W is a valid QC-QC
...
% Written by Alastair Abbott 2022, last modified 22 September 2022
    ...
    if isa(W,'sdpvar')
        is_valid_QCSup = [trace(W) == d_O, superop_in_QCSup_cone(Wr,dims,parties)];
    else
        % input is an explicit W; check the random robustness
        is_valid_QCSup = (abs(trace(W) - d_O) <= tol) ...
            && superop_random_robustness(Wr,dims,parties,'QCSup',sdpsettings('solver','mosek','verbose',0)) < tol;
    end
end
